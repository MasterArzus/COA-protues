...
// 时间参数
int yy = 0, hh = 0, mm = 0, ss = 0, dd = 0, bb = 0, set_day;
String Day = "  ";
...
void blinking(){
  if (set_counter_counter==0){
    //间隔为2，不包括Curser位置
    if (set_alarm_counter ==0 && set_timer_counter == 1){lcd.setCursor(0,0);  lcd.print("  ");}
    if (set_alarm_counter ==0 && set_timer_counter == 2){lcd.setCursor(3,0);  lcd.print("  ");}
    if (set_alarm_counter ==0 && set_timer_counter == 3){lcd.setCursor(6,0);  lcd.print("  ");}

    if (set_alarm_counter ==0 && set_timer_counter == 4){lcd.setCursor(1,1);  lcd.print("   ");}
    if (set_alarm_counter ==0 && set_timer_counter == 5){lcd.setCursor(5,1);  lcd.print("  ");}
    if (set_alarm_counter ==0 && set_timer_counter == 6){lcd.setCursor(8,1);  lcd.print("  ");}
    if (set_alarm_counter ==0 && set_timer_counter == 7){lcd.setCursor(11,1); lcd.print("    "); }
...
void update () {
  ...
  if(digitalRead (bt_alarm) == 0 && SETTING==0){ 
    ...
    if(set_counter_counter==0 && set_counter_counter == 0){
  ...
  if(digitalRead (bt_up) == 0){         
    ...
    if (set_counter_counter==0){
      ...
      if (set_alarm_counter==0 && set_timer_counter==4)set_day=set_day+1;
      if (set_alarm_counter==0 && set_timer_counter==5)dd=dd+1;
      if (set_alarm_counter==0 && set_timer_counter==6)bb=bb+1;
...
void counterUpdate(){
  ...
  {
    lcd.setCursor(0,0);  //Show "TIME" on the LCD
    lcd.print(" Real Time Clock ");delay (2000);
  ...
  {
    lcd.setCursor(0,0);  //Show "TIME" on the LCD
    lcd.print(" Real Clock ");delay (2000);
...
void cdDiffToDigit()
{
  cdHH = cdDifferenceTime / 3600;
  cdMM = (cdDifferenceTime % 3600) / 60;
  cdSS = (cdDifferenceTime % 60);
}

void cdConfirm()
{
  t = rtc.getTime();
  cdDifferenceTime = cdHH * 3600 + cdMM * 60 + cdSS;
  cdExpectedTime = (t.hour,DEC) + (t.min,DEC)+ (t.sec,DEC) + cdDifferenceTime;
}
...
void cdLogicFlow()
{
  ...
  if (cdDifferenceTime > 0)
  {
    if (cdStop == false) // Running
      cdDifferenceTime = cdExpectedTime - ((t.hour,DEC) + (t.min,DEC)+ (t.sec,DEC));
  }
